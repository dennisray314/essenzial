...
if ( ! function_exists( 'print_attribute_option' ) ) {
	function print_attribute_option( $selected_value, $value, $label, $name ){
		//$selected = sanitize_title( $selected_value ) === $selected_value ? checked( $selected_value, sanitize_title( $value ), false ) : checked( $selected_value, $value, false );
		...
		printf('<option value="%1$s" %2$s>%3$s</option>',$id,$selected,$label);
	}
}
...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->id ); ?>" data-product_variations="<?php echo htmlspecialchars( json_encode( $available_variations ) ) ?>">
	...
	if ($isMobile && !isIOS()) : ?>
			<?php foreach ( $attributes as $name => $options ) : ?>
				<select id="mobile-select-variation">
				<option selected disabled>Scegli <?php echo wc_attribute_label( $name ); ?></option>
				<?php
						$sanitized_name = sanitize_title( $name );
						if ( isset( $_REQUEST[ 'attribute_' . $sanitized_name ] ) ) {
							$selected_value = $_REQUEST[ 'attribute_' . $sanitized_name ];
						} elseif ( isset( $selected_attributes[ $sanitized_name ] ) ) {
							$selected_value = $selected_attributes[ $sanitized_name ];
						} else {
							$selected_value = '';
						}
						...
						if ( ! empty( $options ) ) {
							if ( taxonomy_exists( $name ) ) {
								...
								foreach ( $terms as $term ) {
									...
									print_attribute_option( $selected_value, $term->slug, $term->name, $sanitized_name );
								}
							} else {
								foreach ( $options as $option ) {
									print_attribute_option( $selected_value, $option, $option, $sanitized_name );
								}
							}
						}
				?>
...
<script>
	jQuery(document).ready(function($){
		...
		$('#mobile-select-variation').on('change',function(){
			var id = '#' + $(this).val();
			$(id).trigger('click');	
		});
	});
