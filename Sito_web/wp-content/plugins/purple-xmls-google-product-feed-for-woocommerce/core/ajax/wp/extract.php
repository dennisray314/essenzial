...
$customToken = "ExportFeedToken";
if ($_REQUEST['token'] == $customToken && $_REQUEST['action'] == "raw") {
    $obj = new RawExportFeed();
}
...
if ($request == 'products') {
    foreach ($result->results as &$this_result) {
        ...
        //WooCommerce-Product
        if (($wc_version[0] <= 2)) {
            $product = get_product($this_result->ID); //WooCommerce - get product by id
        } else {
            $product = new WC_Product($this_result->ID);
        }
        //Links
...
function insertVariation($id, $source, &$resultlist)
{
    ...
    if ($image_id) {
        ...
        if ($var_image[0]) {
            $this_result->variation_image = $var_image[0];
        }
