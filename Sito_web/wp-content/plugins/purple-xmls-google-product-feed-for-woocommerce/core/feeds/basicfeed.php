...
class PBasicFeed
{
    ...
    protected function createFeedCustom($file_name, $file_path, $remote_category)
    {
        //$file_name is (incorrectly) a url due to an unfortunate nomenclature left over from v2.x
        $this->fileHandle = fopen($this->filename, "w");
        if ($this->has_header) {
...
#echo '<pre>';print_r($this->errors);die;
    ...
    public function getFeedData($category, $remote_category, $file_name, $saved_feed = null, $miinto_country_code = null)
    {
    ...
    public function getCustomFeedData($file_name, $saved_feed = null, $miinto_country_code = null)
    {
        ...
        */
        ...
        if ($this->auto_update_feedlist) {
            PFeedActivityLog::updateCustomFeedList($cpf_custom_local_category, $cpf_custom_remote_category, $file_name, $file_path, $this->providerName, $this->productCount, $miinto_country_code);
        }
    ...
    {
        $this_product->attributes['current_category'] = $this_product->attributes['category'];
