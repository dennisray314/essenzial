...
function submitForm(service_name, type) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            console.log(limit);
            console.log(data.count - lowerlimit - 10);
...
function showSelectedProductTables(feed_id, $_data, edit = false) {
    if (edit == true) {
        jQuery('#cpf-product-search-results').show();
    }
    jQuery.ajax({
        ...
        success: function (res) {
            ...
            // jQuery('.feed-options').show();
            jQuery("#cpf-the-list_1").html((res));

            var $moved_data = jQuery('#cpf-the-list_1 tr').length;
            if ($_data > $moved_data) {
                jQuery('#cpf_move_selected').trigger('click');
            } else {
                jQuery('#ajax-loader-cat-import').hide();
            }
        }
