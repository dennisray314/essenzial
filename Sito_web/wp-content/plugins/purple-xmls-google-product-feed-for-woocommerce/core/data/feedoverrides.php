...
class PFeedOverride extends PBaseFeedOverride
{
    ...
    function __construct($providerName, $parent, $saved_feed)
    {
        ...
        if (count($this->advancedCommands) > 0) {
                ...
                switch ($command) {
                    ...
                    case 'setattributeparam':
                        ...
                        if ($recent_attribute != null){
                            $attr = $params[2];
                            $attr_value = $params[3];
                            $recent_attribute->$attr = $attr_value;
                        }
                        break;
                    case 'setlicensekey':
                        $reg = new PLicense();
        ...
        if (count($this->advancedCommandsCustom) > 0) {
            $limit = '';
            if (isset($GLOBALS['_POST']['feedLimit']))
                $limit = $GLOBALS['_POST']['feedLimit'];
            ...
            if (is_array($this->advancedCommandsCustom)) {
                foreach ($this->advancedCommandsCustom as $key => $this_option) {
