...
function wplc_pro_triggers_page(){
	wplc_enqueue_admin_styles();
	$wplc_add_trigger_btn = isset($_GET['wplc_action']) ? "" : "<a href='?page=wplc-pro-triggers&wplc_action=add_trigger' class='wplc_add_new_btn'>". __("Add New", 'wp-livechat') ."</a>";
    ...
    if(isset($_GET['wplc_action']) && ($_GET['wplc_action'] == "add_trigger" || $_GET['wplc_action'] == "edit_trigger")){
		wplc_pro_get_add_trigger_content();
    } else if(isset($_GET['wplc_action']) && ($_GET['wplc_action'] == "delete_trigger")){
    	wplc_pro_get_delete_trigger_content();
    } else {
    	wplc_pro_get_trigger_table();
    }
...
function wplc_pro_get_add_trigger_content(){
	wplc_pro_trigger_admin_js();
	...
	//Header Handler
	$header_array = wplc_pro_trigger_admin_head();
	if($header_array){
    ...
    wp_editor( (isset($trigger_content["html"]) ? wplc_shortcode_filter($trigger_content["html"], true) : ""),"wplc_trigger_content",array("teeny" => false, "media_buttons" => true, "textarea_name" => "wplc_trigger_content", "textarea_rows" => 5));
    ...
    $content .= 	"<tr>";
    $content .= 		"<td></td>";
    $content .= 		"<td><input class='button button-primary' type='submit' name='wplc_trigger_submit' value='".__($wplc_submit_label, "wplivechat")."'> <a href='".admin_url()."admin.php?page=wplc-pro-triggers"."' class='button'>".__("Close", "wplivechat")."</a></td>";
    $content .= 	"</tr>";
...
function wplc_pro_trigger_admin_js(){
	?>
	<script>
...
function wplc_pro_trigger_admin_head(){
	if(isset($_GET['wplc_action'])){
		$return_array = array();
		$form_valid = true;
		if(isset($_POST['wplc_trigger_submit'])){
			...
			if(isset($_POST['wplc_trigger_content']) && $_POST['wplc_trigger_content'] !== ""){	
				$supporter_tags = wplc_trigger_get_allowed_tags();
				$serialized_content['html'] = wp_kses(nl2br(wplc_shortcode_filter($_POST['wplc_trigger_content'])), $supporter_tags);
			}
		...
		if($_GET['wplc_action'] == "add_trigger"){
				if($form_valid && isset($_POST['wplc_trigger_submit'])){
					//All good continue
					if(wplc_add_trigger($return_array)){
						//Redirect here
						echo "<script> window.location = '".admin_url()."admin.php?page=wplc-pro-triggers"."';</script>";
					}
				} else {
			...
			if($form_valid && isset($_POST['wplc_trigger_submit'])){
				...
				if(isset($_GET['trigger_id'])){
					if(wplc_edit_trigger($return_array, intval($_GET['trigger_id']))){
						//Show edit message
			...
			if(isset($_GET['trigger_id'])){
				$trigger_data = wplc_get_trigger(intval($_GET['trigger_id']));
				if($trigger_data){
			...
			if(isset($_GET['trigger_id'])){
				...
				if(isset($_POST['delete_confirm'])){
					//Delete now
					if(wplc_delete_trigger(intval($_GET['trigger_id']))){
						//Success
					}
					echo "<script> window.location = '".admin_url()."admin.php?page=wplc-pro-triggers"."';</script>";
				}
...
function wplc_add_trigger($trigger_data){
	global $wpdb;
...
function wplc_edit_trigger($trigger_data, $trigger_id){
	global $wpdb;
    global $wplc_tblname_chat_triggers;
	if($trigger_data){
		$trigger_name;
...
function wplc_delete_trigger($trigger_id){
	global $wpdb;
	global $wplc_tblname_chat_triggers;
	$trigger_id = intval($trigger_id);
...
function wplc_change_trigger_status($trigger_id, $trigger_status){
	global $wpdb;
    global $wplc_tblname_chat_triggers;
...
function wplc_pro_get_trigger_table(){
	$content = "";

	wplc_pro_trigger_table_head();

	//Now Get the Contents
  	$results = wplc_get_all_triggers();
	
	$conflict_array = wplc_triggers_check_for_conflicts($results);
    ...
    $content .= 		"</tr>";
  	...
  	if($results){
  		foreach ($results as $result) {
  			$trigger_actions = "<a class='button' href='?page=wplc-pro-triggers&wplc_action=edit_trigger&trigger_id=".$result->id."'>".__("Edit", "wplivechat")."</a> ";
  			$trigger_actions .= "<a class='button' href='?page=wplc-pro-triggers&wplc_action=delete_trigger&trigger_id=".$result->id."'>".__("Delete", "wplivechat")."</a> ";
  			...
  			$content .= 	"<td>".__(wplc_get_type_from_code($result->type), "wplivechat")."</td>";
  			...
  			$content .=			  "<a href='?page=wplc-pro-triggers&wplc_action=trigger_status_change&trigger_id=".$result->id."&trigger_status=".($result->status == 1 ? "0" : "1")."' title='".__("Click to change trigger status", "wplivechat")."'>";
...
function wplc_pro_trigger_table_head(){
	if(isset($_GET['wplc_action']) && isset($_GET['trigger_id']) && isset($_GET['trigger_status'])){
		if($_GET['wplc_action'] == 'trigger_status_change'){
			...
			if(isset($trigger_id)){
				...
				//All good execute
				if(wplc_change_trigger_status($trigger_id, $trigger_status)){
...
function wplc_get_all_triggers(){
    global $wpdb;
    global $wplc_tblname_chat_triggers;
...
function wplc_get_trigger($trigger_id){
	global $wpdb;
    global $wplc_tblname_chat_triggers;
...
function wplc_get_active_triggers(){
    global $wpdb;
    global $wplc_tblname_chat_triggers;
...
function wplc_get_type_from_code($code){
	$type_string = "";
	switch($code){
		case 0:
...
function wplc_pro_get_delete_trigger_content(){
	$header_array = wplc_pro_trigger_admin_head();
	$trigger_name = "";
	if($header_array){
	...
	if( (isset($_GET['wplc_action']) & isset($_GET['trigger_id']))&& ($_GET['wplc_action'] == "delete_trigger" && $_GET['trigger_id'] != "")){
		...
		$content .= 				" <a href='".admin_url()."admin.php?page=wplc-pro-triggers' class='button'>".__("Cancel", "wplivechat")."</a>";
		$content .= 			"</td>";
		$content .= 		"</tr>";
	  	$content .= 	"</table>";
	  	$content .= "</form>";
...
add_filter("wplc_filter_hovercard_content","wplc_filter_control_modern_theme_hovercard_content_triggers",10,1);
function wplc_filter_control_modern_theme_hovercard_content_triggers($msg) {
	$unaltered_msg = $msg;
	$post_id = url_to_postid($_SERVER['HTTP_REFERER']); //User Admin-Ajax Referrer
	$matched_trigger = wplc_check_trigger_filters($post_id); //Find first matching trigger for page
	if($matched_trigger != false){
		$trigger_data = wplc_get_trigger($matched_trigger);
		if($trigger_data !== false && is_array($trigger_data)){
			...
			if(intval($trigger_data->show_content) == 1){
				...
				if($unserialized_content){
					$msg = do_shortcode(wplc_shortcode_filter($unserialized_content['html'], true));
				}
			}
		}
	}else{
...
function wplc_check_trigger_filters($page){
	$triggers = wplc_get_active_triggers();
...
function wplc_tirggers_enqueue_user_styles_scritps($trigger_id){
	wp_register_script("wplc_trigger_js",  plugins_url('/js/wplc_user_triggers.js', __FILE__), array('jquery'));
	/*Localize vars here*/
	$trigger_id = intval($trigger_id);

	$trigger_data = wplc_get_trigger($trigger_id);
	if($trigger_data !== false && is_array($trigger_data)){
		$trigger_data = $trigger_data[0];
		//Only Load styles if content is being overriden
		if(intval($trigger_data->show_content) == 1){
			wp_enqueue_style("wplc_trigger_styles",  plugins_url('/css/wplc_trigger_styles.css', __FILE__))	;
		}
...
function wplc_trigger_get_allowed_tags(){
...
function wplc_shortcode_filter($content, $decode = false){
	if($decode){
...
function wplc_triggers_check_for_conflicts($triggers){
