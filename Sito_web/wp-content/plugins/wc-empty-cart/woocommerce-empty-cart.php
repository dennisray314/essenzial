...
function empty_cart_callback() {
	...
	load_plugin_textdomain('wc-empty-cart', false, '');
	
	if (!defined('WC_QUICK_VIEW_URL')) {
		define ('WC_QUICK_VIEW_URL','');
	}
        ...
        function woocommerce_empty_cart_button($cart) {
            ...
            <tr>
                <td colspan="6" class="actions">
                    <?php if (empty($_GET)) { ?>
                        <a class="button emptycart" href="<?php echo $cart_url; ?>?clear-cart=empty-cart"><?php _e('Empty Cart', 'wc-empty-cart'); ?></a>
                    <?php } else { ?>
                        <a class="button emptycart" href="<?php echo $cart_url; ?>&clear-cart=empty-cart"><?php _e('Empty Cart', 'wc-empty-cart'); ?></a>
                    <?php } ?>
