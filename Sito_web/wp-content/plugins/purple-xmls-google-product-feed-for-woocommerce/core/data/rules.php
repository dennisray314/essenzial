...
//***********************************************************
    ...
    class PFeedRuleDiscount extends PFeedRule
    {
        ...
        public function process($product)
        {
            ...
            $product->attributes['original_regular_price'] = $product->attributes['regular_price'];
...
//***********************************************************
    ...
    class PFeedRuleAddPriceFlatFee extends PFeedRule
    {
        ...
        public function initialize()
        {
            ...
            */
            ...
            //Additive value
            if (!isset($number_value) OR $number_value < 0) $number_value = 0;
        ...
        public function process($product)
        {
            ...
            if (count($this->parametersV) == 3) {
                if (!isset($product->attributes[$this->parametersV[0]])) {
                    ...
                    if ($findSet_pos !== false) { //if 1st parameter is found in 0th attribute
        ...
        public function process($product)
        {
            ...
            if (isset($product->attributes['sale_price'])) {
                ...
                elseif ($product->attributes['sale_price'] == '')
                    unset($product->attributes['sale_price']); //unset sale_price, since rapidcart will still map an empty attribute
            }
        }
    ...
    class PFeedRulePricerounding extends PFeedRule
    {
        ...
        public function process($product)
        {
            ...
            if($product->attributes['original_regular_price']>0)
                $product->attributes['original_regular_price'] = number_format((double)$product->attributes['original_regular_price'], $this->digits, '.', '');
        }
...
//***********************************************************
    class PFeedRuleRemoveProductByAttributeCondition extends PFeedRule
    {
...
//warning: should only be applied once
    ...
    class PFeedRuleTSVStandard extends PFeedRule
    {
        //csvstandard should also elminiate characters outside \x20-\x7E ...what about other language chars? -cg
        public function initialize()
        {
...
//Try to find identifiers (upc, brand, mpn) within short description
    class PFeedRuleFindIdentifiers extends PFeedRule
    {
...
//places the subcategory into the 'subcategory' attribute
    class PFeedRuleGetSubcategory extends PFeedRule
    {
...
//extracts featured images and gallery images in piped format
    class PFeedRuleGetPipedImages extends PFeedRule
    {
...
// Removes 0 quantity products if stock is managed
    class PFeedRuleExcludeoos extends PFeedRule
    {
        ...
        public function process($product)
        {
            if (strtolower($product->attributes['manage_stock']) == 'yes') {
    ...
    class PFeedRuleStockJH extends PFeedRule
    {
        ...
        public function process($product)
        {
            ...
            if ($managing_stock && $backorders_allowed)
                $product->attributes['stock_status'] = 'In Stock';
            elseif ($managing_stock && $product->attributes['_stock'] <= 0) //$this->woocommerce_notify_no_stock_amount)
                $product->attributes['stock_status'] = 'Out of Stock';
            else {
    ...
    class PFeedRuleHideStockJH extends PFeedRule
    {
        ...
        public function process($product)
        {
            ...
            if ($managing_stock && $product->attributes['_stock'] <= 0) //$this->woocommerce_notify_no_stock_amount)
                $product->attributes['valid'] = false;
            else {
                if ($product->attributes['_stock_status'] == 'instock' || $product->attributes['_stock_status'] == '1') {
    ...
    class PFeedRulePriceConversion extends PFeedRule
    {
        ...
        public function initialize()
        {
            ...
            $this->digits=$this->parameters[3]?$this->parameters[3]:2;
...
class PFeedRuleRemovehtmlentities extends PFeedRule
{
    public function initialize()
    {
        parent::initialize();
